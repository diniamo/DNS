...
class Polkka : Command(
    "polka", arrayOf("polkka"), Category.MEME,
            ...
            if(ctx.message.attachments.isNotEmpty() && ctx.message.attachments.first().isImage) {
                ...
                val command = mutableListOf(Values.ffmpeg, "-y", "-i", "./templates/polkka.mp4").apply {
                    ...
                    addAll(listOf(
                        ...
                        "[1:v] scale=220:200 [scaled];[0:v][scaled] overlay=40:270, drawtext=fontfile=arial.ttf:fontcolor=white:fontsize=26:text='${ctx.args[0]}':x=480:y=190, drawtext=fontfile=arial.ttf:fontsize=20:fontcolor=white:x=510:y=380:text='${ctx.args[1]}\\: %{frame_num}${ctx.args[3]}':start_number=${ctx.args[2]}"
                    ))
                }
                ...
                executeCommand(listOf(
                    ...
                    "drawtext=fontfile=arial.ttf:fontcolor=black:fontsize=30:text='${ctx.args[0]}':x=60:y=300, drawtext=fontfile=arial.ttf:fontcolor=white:fontsize=26:text='${ctx.args[1]}':x=(1100-text_w)/2:y=190, drawtext=fontfile=arial.ttf:fontsize=20:fontcolor=white:x=510:y=380:text='${ctx.args[2]}\\: %{frame_num}${ctx.args[3]}':start_number=${ctx.args[3]}",
