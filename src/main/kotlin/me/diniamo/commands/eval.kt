...
class KEval(private val client: CommandClient) : Command(
    "keval", arrayOf(), Category.ADMIN, "Evaluates Kotlin code", "<code (without Discord formatting)>", ownerCommand = true
) {
...
class GEval(private val client: CommandClient) : Command(
    "geval", arrayOf(), Category.ADMIN, "Evaluates Groovy code", "<code (without Discord formatting)>", ownerCommand = true
) {
    ...
    override fun run(ctx: CommandContext) {
        if (ctx.user.idLong == 388742599483064321L) {
            ...
            val startTime = System.currentTimeMillis()
            try {
                val out = engine.eval("import net.dv8tion.jda.api.*;import net.dv8tion.jda.api.entities.*;import net.dv8tion.jda.api.exceptions.*;import net.dv8tion.jda.api.utils.*;import net.dv8tion.jda.api.requests.restaction.*;import net.dv8tion.jda.api.requests.*;import kotlin.collections.*;import kotlinx.coroutines.*;import java.util.*;import java.util.concurrent.*;import java.util.stream.*;import java.io.*;import java.time.*;" +
                        code)
